...
void UPointTargetProjectileSpawnData::Shoot(UWorld* world, ACharacter* ShootingCharacter, AWeapon* weapon, FVector aimDirection, float AimDistance, float buttonHeldTime)
{
	...
	TArray<AProjectile*> bullets;
	auto aimPoint = GetMouseAimPosition(world, hitNormal);

	if (SpawnBulletOnAimPoint)
		bullets = SpawnBullets(world, aimPoint,  (hitNormal.Rotation() - FRotator(90, 0, 0)));
	else bullets = SpawnBullets(world, weapon->ShootPoint->GetComponentTransform(), weapon->ShootPoint->GetRightVector());
